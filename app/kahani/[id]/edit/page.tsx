...
function EditArticleContent({ id }: { id: string }) {
  ...
  useEffect(() => {
    async function fetchArticle() {
      try {
        const res = await fetch(`/api/kahani/${id}`);
        if (res.ok) {
  ...
  return (
    <div className="flex min-h-screen w-full" style={{ background: "var(--bg-primary)" }}>
      ...
      <main className="flex-1 lg:ml-[260px] pb-20 lg:pb-0">
        ...
        >
          <div>
            <h1 className="text-lg font-bold" style={{ color: "var(--text-primary)" }}>Edit Kahani</h1>
        ...
        <div className="px-6 lg:px-8 py-6 max-w-7xl">
          {loading ? (
            ...
            >
              ...
              <p className="text-sm" style={{ color: "var(--text-muted)" }}>This kahani may have been deleted.</p>
            </div>
          ) : (
              <NewsForm mode="edit" initialData={article!} apiEndpoint="/api/kahani" redirectPath="/kahani" />
          )}
        </div>
      </main>
    </div>
